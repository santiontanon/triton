...
public class ConvertPatternsToAssembler {
            ...
            if (differentColors.size()==1) {
                ...
                if (differentColors.get(0) == 0) {
                    ...
                    for(int i2 = i+1;i2<PH;i2++) {
                        List<Integer> pixels_next = patternColors(x, y, i2, image);
    ...
    public static List<Integer> patternColors(int x, int y, int line, BufferedImage image) throws Exception {
        ...
        if (differentColors.size()>2) System.out.println("ERROR "+(ERROR_color_count++)+": more than 2 colors in line " + x*PW + ", " + (y*PH) + "+" + line);
        return pixels;        
    }
