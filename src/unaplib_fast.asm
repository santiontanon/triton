;  Adapted to compile with Glass by Santiago Ontañón
...
LWM0.ReloadByteC0:		
	RELOAD_A 
	jr c,LWM0.Check2ndBit

;
;  case "0"+BYTE: copy a single literal

LWM0.CASE0:			
	COPY_1						; first byte is always copied as literal
...
LWM0.MainLoop:		
	add a,a 
	jr nc,LWM0.CASE0 
	jr z,LWM0.ReloadByteC0	; "0"+BYTE = copy literal
LWM0.Check2ndBit:		
	add a,a 
	jr nc,LWM0.CASE10 
	jr z,LWM0.ReloadByteC1	; "10"+gamma(offset/256)+BYTE+gamma(length) = the main matching mechanism
LWM0.Check3rdBit:		
	add a,a
	call z,ReloadByte
	jp c,LWM1.CASE111	; "110"+[oooooool] = matched 2-3 bytes with a small offset

;
;  branch "110"+[oooooool]: copy two or three bytes (bit "l") with the offset -1..-127 (bits "ooooooo"), or stop

LWM0.CASE110:		; "use 7 bit offset, length = 2 or 3"
	...
	ld ixl,c
	ld ixh,b				; save offset for future LWMs
...
LWM0.ReloadByteC1:
	RELOAD_A
	jr c,LWM0.Check3rdBit

;
;  branch "10"+gamma(offset/256)+BYTE+gamma(length): the main matching mechanism

LWM0.CASE10:		; "use a gamma code * 256 for offset, another gamma code for length"
	call GetGammaCoded
...
LWM0.Add1:			
	inc bc
LWM0.Add0:	; for offs<128 : 4+4+7+7 + 4+7 + 8+7 + 6+6 = 60t
			; for offs>=1280 : 4+4+7+12 + 6 = 33t
			; for 128<=offs<1280 : 4+4+7+7 + 4+12 = 38t OR 4+4+7+7 + 4+7+8+12 = 53t

LWM0.CopyMatch:		; this assumes that BC = len, DE = dest, HL = offset
...
LWM1.MainLoop:		
	add a,a
	jr nc,LWM0.CASE0
	jr z,LWM1.ReloadByteC0		; "0"+BYTE = copy literal
LWM1.Check2ndBit:
	add a,a
	jr nc,LWM1.CASE10
	jr z,LWM1.ReloadByteC1		; "10"+gamma(offset/256)+BYTE+gamma(length) = the main matching mechanism
LWM1.Check3rdBit:		
	add a,a
	call z,ReloadByte
	jr nc,LWM0.CASE110		; "110"+[oooooool] = matched 2-3 bytes with a small offset
...
LWM1.CASE111:		
	...
	add a,a
	call z,ReloadByte
	rl c	; read short offset (4 bits)
	ENDM
	ex de,hl
	jr z,LWM1.WriteZero		; zero offset means "write zero" (NB: B is zero here)
...
LWM1.WriteZero:
	...
	NEXT_HL
	ex de,hl
	jp LWM0.MainLoop		; 10+4*(4+10+8)+4+7 + 11+15+7+10 + 7+4+6+10 = 179t
...
LWM1.ReloadByteC1:		
	RELOAD_A
	jr c,LWM1.Check3rdBit

;
;  branch "10"+gamma(offset/256)+BYTE+gamma(length): the main matching mechanism

LWM1.CASE10:		; "use a gamma code * 256 for offset, another gamma code for length"
	call GetGammaCoded
...
LWM1.KickInLWM:		; "and a new gamma code for length"
	inc c
	call GetGammaCoded.ReadGamma		; BC = len
...
GetGammaCoded:		
	ld bc,1
GetGammaCoded.ReadGamma:
	add a,a
	jr z,GetGammaCoded.ReloadByteRG1
	rl c
	rl b
	add a,a
	ret nc				; NB: flag NC immediately says we do not need to reload our byte...
	jr nz,GetGammaCoded.ReadGamma			; ...even better, flag NZ then automatically means flag C :)

GetGammaCoded.ReloadByteRG2:
	RELOAD_A
	ret nc
	jr GetGammaCoded.ReadGamma

GetGammaCoded.ReloadByteRG1:
	RELOAD_A
	rl c
	rl b
	add a,a
